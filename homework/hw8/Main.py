...
if __name__ == '__main__':
    ...
    parser.add_argument('--niter', type=int, default=2000000,
    ...
    parser.add_argument('--train_freq', type=int, default=2,
                        help='train every train_freq iterations')
    parser.add_argument('--train_start', type=int, default=5000,
                        help='train start iteration')
    parser.add_argument('--batch_size', type=int, default=32,
                        help='SGD batch size')
    parser.add_argument('--discount', type=float, default=0.99,
                        help='discount factor (or gamma)')
    parser.add_argument('--replay_size', type=int, default=1000000,
                        help='for DQN, replay buffer size')
    parser.add_argument('--target_update', type=int, default=2500,
                        help='for DQN, update target net every target_update\
                            grad steps')
    # Difference between -v0, -v4, Deterministic, NoFrameSkip, etc. on
    ...
    parser.add_argument('--frame_stack', type=int, default=4,
                        help='for Atari, stack 4 consecutive frames together\
                            as observations')
    parser.add_argument('--eps_decay', type=int, default=200000,
                        help='for DQN, epsilon-greedy exploration over\
                            eps_decay iterations')
    parser.add_argument('--ent_coef', type=float, default=0.01,
                        help='for A2C, coefficient of entropy')
    parser.add_argument('--value_coef', type=float, default=0.5,
                        help='for A2C, coefficient of value function loss')
    parser.add_argument('--print_freq', type=int, default=500)
    parser.add_argument('--checkpoint_freq', type=int, default=200000)
    parser.add_argument('--save_dir', type=str, default='breakout_dqn/')
    parser.add_argument('--log', type=str, default='log.txt',
                        help='the log file name, appended to save_dir')
    parser.add_argument('--load', type=str, default='')
    parser.add_argument('--parallel_env', type=int, default=0)
    ...
    if args.parallel_env:
        env = Env.ParallelEnv(args.env, args.nproc, args.frame_skip,
                              args.frame_stack)
    else:
        env = Env.LocalEnv(args.env, args.nproc, args.frame_skip,
                           args.frame_stack)
    ...
    if args.algo == 'dqn':
        ...
        algo = Algo.ActorCritic(
            env.obs_space, env.act_space,
            args.lr, args.nproc, args.train_freq, args.discount, args.ent_coef,
    ...
    obses = env.reset()
    logprint('obses on reset:', len(obses), 'x', obses[0].shape,
             obses[0].dtype)
    ...
    def sigterm_handler(signal, frame):
        ...
        if not sig_reenter:
            ...
            logprint('sigterm received, save checkpoint to',
                     args.save_dir + str(it) + '.pth')
            env.close()
            if len(args.log):
                log_file.close()
        else:
    ...
    for it in range(1, args.niter):
        # results is the format of list of nproc
        # (new_state, reward, terminal) tuple
        # i.e. [(news_1, r_1, t_1), (news_2, r_2, t_2), ...,
        # (news_nproc, r_nproc, t_nproc)]
        ...
        for i, (sn, r, t, (r_raw, true_done)) in enumerate(results):
            ...
            episode_rewards[i] += r
            if t:
                avg_episode_len = exp_moving_average(avg_episode_len,
                                                     episode_lens[i])
                episode_lens[i] = 0
                avg_episode_reward = exp_moving_average(avg_episode_reward,
                                                        episode_rewards[i])
                episode_rewards[i] = 0
                n_episodes += 1
            raw_episode_rewards[i] += r_raw
            if true_done:
                avg_raw_episode_reward =\
                    exp_moving_average(avg_raw_episode_reward,
        ...
        if it % args.print_freq == 0:
            ...
            logprint("iter %6d |loss %6.2f |n_ep %5d |ep_len %6.1f\
                        |ep_rew %6.2f |raw_ep_rew %6.2f |env_step %6d\
                        |time %s rem %s" %
                     (it, loss, n_episodes, avg_episode_len,
                      avg_episode_reward, avg_raw_episode_reward,
                      num_env_steps, time_passed, time_remain))
