    ...
    while True:
        ...
        if cmd == 'step':
            ...
            if atari:
                ...
                if t:
                    s = reset()  # override existing terminal state
...
class ParallelEnv:
    def __init__(self, name, nproc, frame_skip, frame_stack):
        self.name = name
        self.nproc = nproc
        self.parent_conns, self.child_conns = zip(*[mp.Pipe() for _ in
                                                    range(nproc)])
        self.procs = [mp.Process(target=process_env,
                                 args=(i, self.child_conns[i], name,
    ...
    def step(self, actions):
        ...
        for e, a in enumerate(actions):
            ...
            if self.atari:
                ...
                s = s.astype(np.float32)
                if t:
                    s = self.reset_e(e)  # override existing terminal state
