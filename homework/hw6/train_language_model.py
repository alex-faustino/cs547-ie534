...
for epoch in range(no_of_epochs):
    ...
    for i in range(0, len(x_train), batch_size):
        ...
        x_input = torch.LongTensor(x_input).to(device)
        ...
        acc = float(np.sum(prediction==x_input.to("cpu").data.numpy()[:,1:]))/sequence_length
        epoch_acc += acc
        epoch_loss += loss.data.item()
        epoch_counter += batch_size

        if (i+batch_size) % 1000 == 0 and epoch==0:
           print(i+batch_size, acc/batch_size, loss.data.item(), norm, "%.4f" % float(time.time()-time1))
    ...
    if((epoch+1)%1==0):
        ...
        for i in range(0, L_Y_test, batch_size):
            ...
            epoch_counter += batch_size
            if (i+batch_size) % 1000 == 0 and epoch==0:
               print(i+batch_size, acc/batch_size)
        epoch_acc /= epoch_counter
        epoch_loss /= (epoch_counter/batch_size)
